...
{
  size_t current= 0;
  int curr_json_depth_limit= thd_get_json_depth();
  struct json_norm_value **stack;
  int err= 0;
  DYNAMIC_STRING key;

  stack = (struct json_norm_value **)malloc(thd_get_json_depth() * sizeof(struct json_norm_value *));
  memset(stack, 0, thd_get_json_depth() * sizeof(struct json_norm_value *));
  stack[current]= root;
                ...
                const char *s, size_t size, CHARSET_INFO *cs)
{
  ...
  je.stack= (int*)malloc(thd_get_json_depth() * sizeof(int));
  memset(je.stack, 0, thd_get_json_depth() * sizeof(int));
