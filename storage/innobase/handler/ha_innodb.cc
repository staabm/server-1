...
static inline size_t min_buffer_pool_size()
{
...
static MYSQL_SYSVAR_SIZE_T(buffer_pool_size, *reinterpret_cast<size_t*>
...
static int innodb_init_params()
{
  ...
  min= min_buffer_pool_size();
  size_t innodb_buffer_pool_size= buf_pool.size_in_bytes_requested;
  if (size_t max= buf_pool.size_in_bytes_max)
    MYSQL_SYSVAR_NAME(buffer_pool_size).max_val= max;
  else if (innodb_buffer_pool_size < SIZE_T_MAX / 2)
    buf_pool.size_in_bytes_max= innodb_buffer_pool_size * 2;
...
#endif
  srv_lock_table_size = 5 * buf_pool.curr_size();// FIXME
  DBUG_RETURN(0);
}
...
static ulong innodb_buffer_pool_chunk_size;

static MYSQL_SYSVAR_ULONG(buffer_pool_chunk_size,innodb_buffer_pool_chunk_size,
	...
	struct st_mysql_value*		value)
{
	...
	if (buf_pool.size_in_bytes_requested != buf_pool.curr_pool_size()) {
		mysql_mutex_unlock(&buf_pool.mutex);
		my_printf_error(
			ER_WRONG_ARGUMENTS,
	...
	if (size_t(intbuf) == buf_pool.curr_pool_size()) {
		mysql_mutex_unlock(&buf_pool.mutex);
		/* nothing to do */
		return(0);
	}

	buf_pool.size_in_bytes_requested = size_t(intbuf);
	mysql_mutex_unlock(&buf_pool.mutex);
	return(0);
}
